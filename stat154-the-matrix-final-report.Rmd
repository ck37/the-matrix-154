...
This project consists of an automated text categorization task (Sebastiani 2002) applied to text documents collected from Project Gutenberg, a free ebook archive. The documents are categorized into 4 genres: “Child”, “History”, “Religion”, and “Science”. The goal of this project is to develop a machine learning classifier to correctly categorize the texts as accurately as possible. Our primary feature set is a word-feature matrix which counts the number of a times each word occurs in a document, known as a bag of words model. We extend this word feature matrix with bigrams, trigrams, and meta-features related to statistics on word and character usage. We use support vector machine and random forest methods, test on new text files, and measure the test error of each method. Our final model is a gradient boosted machine.
...
We also removed the lengthy header and footer text created by Project Gutenberg that would likely hurt our predictive accuracy. A set of documents, roughly 600 or more, consisted entirely of Project Gutenberg legal or other non-literary text. We remove some of those documents although would have liked to spend more time removing the remainder (e.g. using outlier detection methods) so that their feature distributions would not influence the prediction algorithms.
...
```{r, echo=F, message=F, warning=F}
df2 = data.frame("Power Feature"=c("words_count","chars_count","words_avg_length","words_distinct","sd_words","word_diversity"),
                "Definition"=c("Number of words","Number of characters","Average words length (Number of characters/Number of words)","Number of words that appears in a text at least once","Standard deviation of word length","Word diversity (Number of distinct words/Number of words)"))
kable(df2)
```
 
(2) Sentence-based power features: We first splitted the text files into sentences and then collected 6 power features by inspecting the list of sentences.

```{r, echo=F, message=F, warning=F}
df3 = data.frame("Power Feature"=c("sentence_count","sentence_avg_length","4digit_nums","digit_count","question_marks","exclamation_points"),
                "Definition"=c("Number of sentences","Average length of sentences","Number of 4-digit number","Number of digits","Number of question marks","Number of exclamation points"))
kable(df3)
```

(3) N-grams: The N-gram is a sequence of N words sliced from a longer string. The word features we described above are unigrams. It is known that the N-gram-based approach to text categorization is tolerant of textual errors, fast and robust. This approach works very well for language classification (William & John, 1994). In this project, for better prediction, other than unigrams we also included the 4736 bigram and 497 trigram features. We were able to make a feature matrix of N-grams with the DocumentTermMatrix() command as we did for the word features but by adjusting the command with the N-gram-tokenizer.

As a result, we have 5245 power features in total. The power feature matrix has dimension 22305 * 5245.
