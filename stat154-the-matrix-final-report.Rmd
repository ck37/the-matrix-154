...
author: |
    ...
    | Dongkyun Kim (22874163), Saejin Kim (21086782),
...
This shows that the word, sentence, and document-based statistics were helpful, as were the bigrams. One trigram showed up in the top 50 features: "made up mind", suggesting that the trigams provided some performance boost, if small.

### Support Vector Machine Model 

Using only the power feature matrix we were able to achieve an accuracy of 70% using SVM. The optimal cost was estimated to be 10 again. Clearly SVM benefits from the word feature matrix in addition to the power features. Per-class accuracies are -- child: 84%, history: 57%, religion: 55%, science: 84%.
...
Now we combine our word and power features and estimate the accuracy using cross-validation.

### Random Forest Model 

We achieve a final accuracy of 88% using both word and power features. Per-class accuracies are -- child: X%, history: X%, religion: X%, science: X%.
...
We submitted our model to the Kaggle competition leaderboard and achieved an overall accuracy of 89.0%. This was much lower than our cross-validation results suggested we would achieve, although consistent with applying the GBM model to the practice set. We believe that our verfication code contains an error, probably in generating or merging the power feature matrix, which prevents the model from leveraging the calculated features as we would expect. Our accuracy was also likely limited by our inability to remove the documents that consisted entirely of Project Gutenberg meta-text, which we unfortunately ran out of time to fully remove from our training data.
...
For source code management we used a private repository on github.com combined with RStudio's built-it git interface. While it took some setup and getting used to, this provided a helpful way to integrate work from multiple team members and was a great success overall. 
